 ...
 * Drifts like the neighboring classics strip (same timing, pause-on-interact).
 */
export function AdaptedStrip() {
  ...
  const items = useMemo(
    () => stripItems(ADAPTED_WORKS, visit, count),
    [visit, count],
  );

  const grow = useCallback(() => {
    setCount((current) => nextStripCount(current, ADAPTED_WORKS.length));
  }, []);
  ...
  return (
    <nav ref={root} className="cell-adapted" aria-label="Adapted by Salon">
      <span className="adapted-kicker type-kicker">Adapted by Salon</span>
      <div
        ref={scroller}
        className="adapted-scroller"
        onScroll={(event) => {
