...
import { startCookies } from "./start-cookies.server";
...
// Self-hosted Vercel (production + preview) — not injected by the Grok deployer.
...
const baseURL = explicitBaseURL ?? {
  // Include loopback + Vercel hosts so dynamic baseURL resolves outside Grok
  // preview (not only *.grok-sandbox.com).
  allowedHosts: [...deployAllowedHosts, "localhost", "127.0.0.1", "[::1]"],
  // `auto` → trust both http:// and https:// expansions of allowedHosts
  // (preview/Vercel are https; local dev is http).
...
// Always include Vercel/preview wildcards so email/password works on
// *.vercel.app even when BETTER_AUTH_URL is unset or points only at production.
const trustedOrigins: string[] = [
  ...(explicitBaseURL ? [explicitBaseURL] : []),
  // Host wildcards (matched against Origin's host)
  ...deployAllowedHosts,
  // Full-origin wildcards (matched against Origin)
  ...deployAllowedHosts.flatMap((host) => [`https://${host}`, `http://${host}`]),
  ...LOCAL_DEV_ORIGINS,
];
...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    // last so it runs after every other plugin's hooks. Local wrapper: the
    // stock plugin destructures a dynamic `setCookie` import and 500s guest
    // server functions when that module is undefined in the Nitro bundle.
    startCookies(),
  ],
});
