...
export async function getSessionUser(
  ...
  try {
    ...
    // Read-only: cookie-cache refresh writes Set-Cookie, and Better Auth's
    // TanStack cookie plugin used to crash the whole server fn when `setCookie`
    // was missing (`Cannot destructure property 'setCookie'`). Guests creating
    // clubs must not depend on that write path.
