...
test("2026-09-17 LE binds open at story start, not chrome", () => {
  ...
  for (const [id, want] of Object.entries(expect)) {
    ...
    if (id === "mr-fortunes-maggot") {
      assert.match(
        packed.breaths.map((b) => b.text).join(" "),
        /call to go to the island of Fanua/,
      );
    }
...
test("full local novels have no stub opening; hydrateLocal serves the complete bind", () => {
  const expect = {
    ...
    "mr-fortunes-maggot": {
      ...
      year: 1927,
      opening: /^Though the Reverend Timothy Fortune/,
      breaths: 728,
      scenes: 19,
    },
    "cousin-betty": {
    ...
    rosmersholm: {
      ...
      author: "Henrik Ibsen",
      year: 1886,
      opening: /^\(SCENE--The sitting-room at Rosmersholm/,
      breaths: 1376,
      scenes: 4,
    },
    salome: {
      ...
      author: "Oscar Wilde",
      year: 1893,
      opening: /^How beautiful is the Princess Salomé to-night!/,
      breaths: 790,
      scenes: 1,
    },
    "the-crux": {
    ...
    "the-napoleon-of-notting-hill": {
      ...
      year: 1904,
      opening: /^The human race, to which so many of my readers belong/,
      breaths: 1117,
      scenes: 15,
    },
    "the-pit": {
      ...
      author: "Frank Norris",
      year: 1903,
      opening: /^At eight o'clock in the inner vestibule of the Auditorium Theatre/,
      breaths: 2848,
      scenes: 10,
    },
    "the-village": {
