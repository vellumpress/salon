...
This tree is a full mirror of Vellum V4 (`c9c405`), including every local catalog text and opening (390 each). Do not thin the catalog to save size.
...
- Chamber reader for all **390 local binds** (texts + openings stay in `src/lib/catalog/texts` and `src/lib/catalog/openings`)
...
390 locally bound works with full text and openings. Years and rights notes come from the V4 shelf. English-off ids stay unlistable as full-text reads.
